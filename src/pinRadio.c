...
void callPython(double frequency, double length);
...
int main() {
    ...
    *(gpio + 0) = gpfsel;
    ...
    printf("Enter the length of antenna in meters (0 for optimal length): ");
    double length;
    if(scanf("%lf", &length) != 1) { perror("Invalid input"); cleanup(0); }
    callPython(baseFrequency, length);
...
void callPython(double frequency, double length) {
    // Run Python script. Can the interpreter be embedded instead?
    char command[256];
    snprintf(command, sizeof(command), "python3 graph.py %.2f %.2f", frequency, length);
...
void cleanup(int sig) {
    ...
    *(clk + (0x70/4)) = GZ_PASSWD | (1 << 5); // Disable clock
    ...
    if(gpio_map) { munmap(gpio_map, BLOCK_SIZE); } // Unmap memory
